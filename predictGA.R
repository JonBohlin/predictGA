...
# Extract all CpGs from trained model
		...
		if(length(temp)>0){
		  cat("Missing essential CpGs needed for prediction\n")
		  cat(predictorSites[temp],"\n")
		  stop("Exiting...")
		}
...
# See if there are any missing values in predictor CpGs
		for(i in predictorSites){
		  ...
		  if(temp>0){
		    cat("Missing values detected in essential CpG\n")
		    cat(i,"\n")
		    stop("Exiting...")
		  }
		}
...
# Place predictor CpGs from original matrix into new matrix
		...
		if( se==TRUE ){
		  ...
		  cat("Using minimum lambda model\n")
		}
...
# Clean up the big stuff, leave the rest for the garbage collector
		rm(mldat, predMatr, temp, allCpGs)
